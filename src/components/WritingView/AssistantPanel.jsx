    ...
    >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {profiles.map((profile) => (
            <button
              ...
              style={{
                padding: '10px 12px',
                border: selectedReaderProfile === profile.id
                  ? `2px solid ${themeObj.accent}`
                  : `1px solid ${themeObj.border}`,
                borderRadius: '8px',
                background: selectedReaderProfile === profile.id
              ...
              onMouseLeave={(e) => {
                e.target.style.borderColor = selectedReaderProfile === profile.id
                  ? themeObj.accent
                  : themeObj.border;
              }}
            >
              <div>{profile.name}</div>
              <div style={{ fontSize: '11px', color: themeObj.textSecondary, marginTop: '2px' }}>
                {profile.description}
              </div>
            </button>
          ))}
        ...
        <div>
          ...
          >
            {currentProfile.guidance.map((tip, idx) => (
              <div key={idx} style={{ marginBottom: idx < currentProfile.guidance.length - 1 ? '6px' : 0 }}>
                {tip}
              </div>
            ))}
          </div>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: `1px solid ${themeObj.border}` }}>
        ...
        >
          <div
            style={{
              background: themeObj.accentLight,
              padding: '8px',
              borderRadius: '6px',
              color: themeObj.text,
            }}
          >
            <div style={{ fontWeight: '600' }}>{wordCount}</div>
            <div style={{ fontSize: '10px', color: themeObj.textSecondary }}>words</div>
          </div>
          <div
            style={{
              background: themeObj.accentLight,
          ...
          >
            <div style={{ fontWeight: '600' }}>{sentenceCount}</div>
            <div style={{ fontSize: '10px', color: themeObj.textSecondary }}>sentences</div>
          </div>
          <div
            style={{
              background: themeObj.accentLight,
          ...
          >
            <div style={{ fontWeight: '600' }}>{avgWordsPerSent}</div>
            <div style={{ fontSize: '10px', color: themeObj.textSecondary }}>avg words/sent</div>
          </div>
          <div
            style={{
              background: themeObj.accentLight,
          ...
          >
            <div style={{ fontWeight: '600' }}>{content.length}</div>
            <div style={{ fontSize: '10px', color: themeObj.textSecondary }}>characters</div>
          </div>
        </div>
      </div>
    </div>
